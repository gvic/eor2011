...
public class StubGenerator {
	...
	public static void main(String[] args) throws Exception {
		...
		if(args.length != 1){
			System.out.println("Vous devez saisir le nom de la classe pour générer son stub :");
			System.out.println("Usage : java StubGenerator <une_classe> (sans l'extension)");
		}
	...
	private static void generateStubClassCode(String[] args) throws Exception {
		...
		String code = "";
		code = code + "public class " + stubClassName + 
		" extends "+superClass+" implements " + itfClassName + ", java.io.Serializable {" + "\n\n\t";
		code = code + "private static final long serialVersionUID = 1L;\n\n\n\t";
		code = code + "/* Constructors */" + "\n\n\t";
		
		Constructor ctorlist[] = Class.forName(superClass).getDeclaredConstructors();
		...
		for(int i = 0; i < ctorlist.length; i++){
			...
			constructorsParameterTypes[i] = new Class[ctorlist[i].getParameterTypes().length];;
		...
		code += "\n\n\t/* Methods */\n\n\t";
		code += generateMethods(Class.forName(itfClassName).getDeclaredMethods(), cName);
	...
	private static String generateMethods(Method[] declaredMethods, String className) {
		String code = "";
		String var = "vloc";
		...
		for(int i = 0; i < declaredMethods.length; i++){
			...
			code += "public "+returnType + " "+method.getName()+"("+declaredParamCode+"){\n\t\t";
			
			String castCode = className + " "+var + " = ("+className+") "+handledObj+";\n\t\t"; 
			
			code += castCode;
			code += returnCode + var +"."+method.getName()+"("+undeclaredParamCode+");\n\t}\n\n\t";
	...
	public static void createNewFileClass(String name, String code) throws IOException{
		// création du fichier nomClasse_stub.java et écriture du texte à l'intérieur
	...
	private static String[] generateArgumentList(Object[] ao, boolean constructor){
		String[] code = {"",""};
		String var = "var";
		String indice = "";
